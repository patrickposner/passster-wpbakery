...
if ( '' === $passster_protection_activation || false == $passster_protection_activation ) {
	$output .= '</div>';
	$output .= $after_output;
	return $output;
}
...
switch ( $passster_protection_type ) {
	case 'password':
		...
		$atts['password'] = $passster_password;
		break;
	case 'captcha':
	...
	case 'passwords':
		...
		$atts['passwords'] = $passster_passwords;
		break;
	case 'password_list':
		$shortcode             = '[passster password_list="' . $passster_password_list . '"';
		$atts['password_list'] = $passster_password_list;
		break;
}

if ( '' !== $passster_api && ! empty( $passster_api ) ) {
	$shortcode  .= ' api="' . $passster_api . '"';
	$atts['api'] = $passster_api;
}

if ( '' !== $passster_headline && ! empty( $passster_headline ) ) {
	$shortcode .= ' headline="' . $passster_headline . '"';
}

if ( '' !== $passster_placeholder && ! empty( $passster_placeholder ) ) {
	$shortcode .= ' placeholder="' . $passster_placeholder . '"';
}

if ( '' !== $passster_button_label && ! empty( $passster_button_label ) ) {
	$shortcode .= ' button="' . $passster_button_label . '"';
}

if ( '' !== $passster_instruction && ! empty( $passster_instruction ) ) {
	$shortcode .= ' instruction="' . $passster_instruction . '"';
}

if ( '' !== $passster_authentication && ! empty( $passster_authentication ) ) {
...
if ( $valid ) {
	...
	$shortcode .= ']';
	$shortcode  = '<div class="description">' . do_shortcode( $shortcode . $output . '[/passster]' ) . '</div>';
	$output     = $shortcode;
}
